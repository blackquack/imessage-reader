...
- The MCP transport is stdio. Codex starts the local server process when needed.
...
The process that opens the database must have macOS Full Disk Access. The MCP server is local and communicates with Codex over stdio; it does not listen on a network port.
